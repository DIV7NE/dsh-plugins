/**
 * Rebuild and reinstall this plugin into a DSH profile, in one command:
 *
 ...
 * cross-drive local dependency in this profile layout: the junction resolves
 * to `<profile>\\D:\\projects\\dshpluginsdev` instead of the real directory, so
 * the launcher cannot read the package manifest, never sees `dsh.bundle.patch`,
 * and never adds the package to `dsh.profile.bundles`. A tarball installs as a
 * real directory and reconciles correctly (verified end to end).
 */
...
runQuietly('dsh plugin --profile ' + profile + ' remove dsh-run-in-terminal')
